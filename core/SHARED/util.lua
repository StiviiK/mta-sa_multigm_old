		...
		if fileExists(file) then
			...
			if file:getSize() > 0 then
				...
				if (f) then
					...
					if (not status) then
						if errmsg then
							local path = split(errmsg, "\"")[2]
							local line = split(errmsg, ":")[2]
							local errormsg = split(errmsg, ":")[3]

							outputChatBox(("(%s) Line: %s:%s"):format(path, line, errormsg))
							error(("(%s) Line: %s:%s"):format(path, line, errormsg), 3)
						end
					end
			...
			error("Error @ 'require' [File not found!] ["..file.."]", 2)
		end
	else
