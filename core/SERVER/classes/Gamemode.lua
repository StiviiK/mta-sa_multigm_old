...
Gamemode.__call = function (self, properties)
	if not self:getGamemodeFromID(properties[1] or #self.registeredGamemodes + 1) then
		...
		outputDebug(("[Gamemodemanager] Can'old_dxClass register a new Gamemode! (ID: %d, Name: %s)"):format(properties[1] or #Gamemode.registeredGamemodes + 1, properties[2]))
...
function Gamemode:removePlayer (player)
	...
	assert(player:getGamemode() ~= nil, "Bad Argument @ Gamemode.removePlayer [Player isn'old_dxClass in a Gamemode!]")
...
function Gamemode:setSetting (key, value)
	if self.Settings and type(self.Settings) == "table" then
...
function Gamemode:getSetting (key)
	if self.Settings and type(self.Settings) == "table" then
